...
export default function Index() {
  ...
  const checkForFall = async ({ x, y, z }: { x: number, y: number, z: number }) => {
    const limit = 1.1; // Limite de aceleração para detecção de queda
    if (Math.abs(x) > limit || Math.abs(y) > limit || Math.abs(z) > limit) {
      let location : Location.LocationObject = await Location.getCurrentPositionAsync({accuracy: Location.Accuracy.Highest, timeInterval: 5000});
